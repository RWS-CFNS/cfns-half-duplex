...
class AISStaticAndVoyageReportMessage(AISMessage):
    def __init__(self, id=5, repeat=0, mmsi=0, ais_version=0, imo=0, callsign=0, shipname=0,
        ...
        super(AISStaticAndVoyageReportMessage, self).__init__({
            ...
            'imo': ["uint", 30, imo],
            'callsign': ["uint", 42,
                         AISString2Bits(callsign, length=42 / 6).int if type(callsign) == str else callsign],
            'shipname': ["uint", 120,
                         AISString2Bits(shipname, length=120 / 6).int if type(shipname) == str else shipname],
            ...
            'destination': ["uint", 120, AISString2Bits(destination, length=120 / 6).int if type(
                destination) == str else destination],
            'dte': ["uint", 1, dte],
            'spare': ["uint", 1, spare]
        })
...
class AISStaticDataReportBMessage(AISMessage):
    def __init__(self, id=24, repeat=0, mmsi=0, partno=1, shiptype=0,
        ...
        super(AISStaticDataReportBMessage, self).__init__({
            ...
            'shiptype': ["uint", 8, shiptype],
            'vendorid': ["uint", 18,
                         AISString2Bits(vendorid, length=18 / 6).int if type(vendorid) == str else vendorid],
            'model': ["uint", 4, model],
...
callsign = "Alfred"
test = AISString2Bits(callsign,length=42/6)
print(test)